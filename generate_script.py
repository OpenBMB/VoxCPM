...
def main():
    ...
    paragraphs, blocks = read_paragraph_blocks(SCRIPT_FILE, MAX_CHARS)
    ...
    for i, block in enumerate(blocks, start=1):
        block_path = os.path.join(OUTPUT_DIR, f"bloque_{i:03d}.wav")
        ...
        if os.path.exists(block_path):
            ...
            reused_count += 1
            print(f"[{i}/{len(blocks)}] Ya existe: {block_path} ({info.duration:.2f}s)")
            continue
    ...
    if fragments:
        ...
        entries = write_srt(srt_entries, SRT_OUTPUT)
